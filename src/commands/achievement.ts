...
export class AchievementCommand extends MeenyCommand
{
  ...
  override async execute(interaction_metadata: Interaction): Promise<void>
  {
    ...
    const backgroundLeft: Canvas.Image = await Canvas.loadImage("./assets/achievement/background/background_left.png");
...
function loadIconList(): CommandOptionChoices
{
  try
  {
    const data: string = fs.readFileSync("./assets/achievement/icon_list.txt", "utf8").trim();
