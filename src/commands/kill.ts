...
export class KillCommand extends MeenyCommand
{
  ...
  constructor()
  {
    ...
    command.setDescription("Kill someone over with no consequences because it's not real life.");
    ...
    try
    {
      const data: string = fs.readFileSync("./assets/kill/messages.txt", "utf8").trim();
